...
export default async function handler(req) {
  ...
  if (airport) {
    try {
      ...
      if (!r.ok) return new Response(JSON.stringify({ok:false,error:r.status}),{status:r.status,headers});
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (ac) return new Response(JSON.stringify({
          ok:true,dep:ac.dep||'',arr:ac.arr||'',from_long:ac.from_long||'',to_long:ac.to_long||'',type:ac.t||'',airline:'',
        }),{headers});
      }
    } catch(e) {}
  ...
  try {
    ...
    const r=await fetch(osUrl,{headers:{'User-Agent':'SkyView/1.0'},signal:AbortSignal.timeout(8000)});
    if (r.ok) {
      const data=await r.json();
